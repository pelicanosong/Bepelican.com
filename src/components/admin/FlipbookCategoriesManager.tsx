...
import {
  useAdminFlipbookCategories,
  useCreateFlipbookCategory,
  useUpdateFlipbookCategory,
  useDeleteFlipbookCategory,
  FlipbookCategoryAdmin,
} from '@/hooks/useAdminFlipbookCategories';
...
import { Plus, Pencil, Trash2, Loader2, Upload, Smile, Image as ImageIcon, X, Mountain, Heart, MapPin, Users, Compass, Sparkles, BookOpen } from 'lucide-react';
...
// Detect if icon value is a URL (image) or emoji/text
const isImageUrl = (icon: string | null) =>
  !!icon && (icon.startsWith('http://') || icon.startsWith('https://'));

const isLucideIcon = (icon: string | null) =>
  !!icon && icon in lucideIconMap;
...
interface CategoryFormState {
  ...
  icon: string; // emoji string or image URL
  iconMode: IconMode;
}
...
export function FlipbookCategoriesManager() {
  const { data: categories, isLoading } = useAdminFlipbookCategories();
  const createCategory = useCreateFlipbookCategory();
  const updateCategory = useUpdateFlipbookCategory();
  const deleteCategory = useDeleteFlipbookCategory();

  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<FlipbookCategoryAdmin | null>(null);
  ...
  const openEdit = (cat: FlipbookCategoryAdmin) => {
  ...
  const renderIconPreview = (icon: string | null) => {
    ...
    if (isLucideIcon(icon)) {
      ...
      return <LucideComp className="h-5 w-5 text-foreground" />;
    }
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            {/* Illustration toggle: emoji or image */}
            <div className="space-y-1.5">
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se eliminarán también las relaciones con flipbooks. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
