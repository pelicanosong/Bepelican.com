#!/usr/bin/env node
/**
 * Añade moni_exchange a PGRST_DB_SCHEMAS en Supabase self-hosted (Contabo) y reinicia REST.
 */
...
patch_line() {
  ...
  if grep '^PGRST_DB_SCHEMAS=' "$f" | grep -q moni_exchange; then
    echo "  · ya incluye moni_exchange: $f"
    return 0
  fi
  sed -i.bak 's/^PGRST_DB_SCHEMAS=\\(.*\\)$/PGRST_DB_SCHEMAS=\\1,moni_exchange/' "$f"
  echo "  ✓ actualizado: $f"
}
...
find /root/supabase -maxdepth 4 -type f \\( -name 'docker-compose.yml' -o -name 'docker-compose.yaml' \\) 2>/dev/null | while read -r yml; do
  if grep -q 'PGRST_DB_SCHEMAS' "$yml" 2>/dev/null && ! grep 'PGRST_DB_SCHEMAS' "$yml" | grep -q moni_exchange; then
    sed -i.bak 's/PGRST_DB_SCHEMAS=\\([^" ]*\\)/PGRST_DB_SCHEMAS=\\1,moni_exchange/g' "$yml"
...
console.log('→ PostgREST: exponer esquema moni_exchange…\n');
...
if (result.status !== 0) {
  console.error('\n✗ No se pudo actualizar PGRST. Editá manualmente PGRST_DB_SCHEMAS y agregá ,moni_exchange');
  process.exit(1);
}
